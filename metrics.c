...
void metrics_print_terminal(int n, const Config *cfgs, const Metrics *ms)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        printf("  %-22s : %ld\n",  "DRAM Loads (total)", m->dram_loads);
        printf("  %-22s : %ld\n",  "DRAM Input Loads",   m->dram_input_loads);
        printf("  %-22s : %ld\n",  "DRAM Weight Loads",  m->dram_weight_loads);
...
void metrics_write_csv(FILE *fp, int n, const Config *cfgs, const Metrics *ms)
{
    ...
    fprintf(fp,
        ...
        "sram_input_bytes,sram_weight_bytes,sram_psum_bytes,sram_total_bytes,"
        "max_PE,"
